...
// Add number cards with two copies of each number for each color
    for (int c = 0; c < 4; c++) { //for the colors
        for (int n = 0; n < 10; n++) { //for the numbers 
            for (int i = 0; i < 2; i++) {
...
void shuffleDeck(card **deck, int *deckSize) {
    for (int i = 0; i < 10000; i++) {
        ...
        card temp = (*deck)[index1]; //it works by swapping pairs indicudally, done 10000 times, using temp vairable to swap
...
// checks and validates if a card can be played on the current top card
int isValidCard(card topOfPile, card candidate, char currentColor) {
...
// Get the actual paired number card from the pile (In our function we have it as second to last played card)
    char pairedNumber = (*pile)[*pileSize-2].name; //gets paired card which is second to last card because now the special card is last
...
// Get the actual paired number card from the pile which is the second-to-last card played
    char pairedNumber = (*pile)[*pileSize-2].name; //paired with second to last card,(so the card it was played with)
    char pairedColor = (*pile)[*pileSize-2].color;
...
// The code above Prints the color 
// Code below displays the players current hand 
void printPlayerHand(player p) {
    printf("%s's hand (%d cards):\n", p.name, p.hand_size); //player name and player handsize, see struct near top
...
// initlazes the structs from before 
...
// Main game loop and it takes turns between players until someone wins or the deck is empty, shouldnt reach tha tpoint though 
    while (!gameOver) {
...
// function checks to find a winner once there is an empty hand        
...
// Just frees / cleans up the dynamic memory and the end of the code    
    for (int i = 0; i < playerCount; i++) {
